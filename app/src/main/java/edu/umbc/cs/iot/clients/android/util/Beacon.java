...
public class Beacon {
    ...
    public static float getTlmBattery(byte[] data) {
        byte version = data[1];
        if (version != 0) {
            Log.w(UMBCIoTApplication.getDebugTag(), "Unknown telemetry version");
            return -1;
        }
    ...
    public static float getTlmTemperature(byte[] data) {
        ...
        if (data[4] == (byte)0x80 && data[5] == (byte)0x00) {
            Log.w(UMBCIoTApplication.getDebugTag(), "Temperature not supported");
            return -1;
        }
